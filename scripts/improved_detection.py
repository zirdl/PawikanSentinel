...
class TurtleDetector:
    def __init__(self, model_path, confidence_threshold=0.45, iou_threshold=0.3):
    ...
    def decode_yolov8_output(self, output_data, input_shape):
        ...
        for i in indices:
            ...
            # 1. Size filtering - turtles have reasonable size bounds
            min_area = 800  # Minimum turtle size in pixels (increased from 400)
            max_area = input_width * input_height * 0.25  # Max 25% of image
            if box_area < min_area or box_area > max_area:
                continue
            
            # 2. Aspect ratio filtering - turtles are roughly oval
            # Typical turtle aspect ratios: 0.7 to 2.2 (width/height)
            if aspect_ratio < 0.6 or aspect_ratio > 2.5:
                continue
            
            # 3. Edge detection filtering - avoid boxes on image edges
            edge_margin = 30
            if (box[0] < edge_margin or box[1] < edge_margin or 
                box[2] > input_width - edge_margin or box[3] > input_height - edge_margin):
                continue
            
            # 4. Position filtering - turtles usually in bottom 70% of frame
            center_y = (box[1] + box[3]) / 2
            if center_y < input_height * 0.3:  # Skip detections in top 30% of image
                continue
            
            # 5. Confidence-based stricter filtering
            if score < 0.5:
                # Stricter requirements for lower confidence detections
                if aspect_ratio < 0.8 or aspect_ratio > 1.8:
                    continue
                if box_area < min_area * 1.5:  # Require larger size for low confidence
                    continue
            
            # 6. Box shape validation - avoid very thin rectangles
        ...
        # Sort by confidence weighted with area (prefer larger, confident detections)
    ...
    def filter_by_motion(self, frame, detections):
        """Filter detections based on motion (turtles should move)"""
        if not detections:
            return detections
        ...
        for det in detections:
            ...
            # Allow detections with some motion OR high confidence
    ...
    def is_consistent_detection(self, detections):
        """Enhanced consistency checking with spatial and temporal awareness"""
        if not detections:
        ...
        # Check if we have detections in at least 3 of last 5 frames
        recent_frames = list(self.detection_history)[-5:]
        frames_with_detections = sum(1 for frame in recent_frames if frame)
        
        if frames_with_detections < 3:
            return False
        
        # Spatial consistency check - look for detections in similar locations
        ...
        for current_center in current_centers:
            ...
            if conf < 0.4:  # Skip very low confidence detections
                continue
            
            consistent_count = 0
            for frame in recent_frames[-4:]:  # Check last 4 frames
                if not frame:
                    continue
                ...
                for prev_center in frame:
                    ...
                    # Stricter spatial and size consistency requirements
                    if distance < 80 and area_diff < 0.5 and pconf > 0.35:
                        consistent_count += 1
                        break
            
            # Need detection in at least 2 of last 4 frames to be consistent
    ...
    def adaptive_confidence(self):
        """Adjust confidence based on time of day"""
        hour = time.localtime().tm_hour
        
        base_confidence = self.confidence_threshold
        
        # Lower threshold during dawn/dusk when lighting is challenging
        if 5 <= hour <= 7 or 18 <= hour <= 20:
            return max(0.35, base_confidence - 0.1)
        
        # Higher threshold during midday when lighting is optimal
        if 10 <= hour <= 16:
            return min(0.65, base_confidence + 0.1)
    ...
    def process_frame(self, frame):
        ...
        cv2.putText(annotated_frame, f"FPS: {fps:.1f} | Inference: {inference_time*1000:.0f}ms", 
    ...
    def draw_detections(self, frame, detections, is_consistent):
        ...
        for i, detection in enumerate(detections):
            ...
            # Color coding: Green for consistent, Orange for detections, Red for low confidence
            if is_consistent:
                color = (0, 255, 0)  # Green
                thickness = 3
            elif confidence > 0.5:
...
def main():
    parser = argparse.ArgumentParser(description="Enhanced Turtle Detection System with False Positive Filtering")
    ...
    # Initialize detector
    detector = TurtleDetector(args.model_path, args.confidence, args.iou_threshold)
    ...
    logger.info("Successfully opened RTSP stream. Processing frames...")
    logger.info("Enhanced filtering enabled: size, aspect ratio, edge, position, motion, and consistency checks")
    ...
    try:
        while True:
            ...
            if args.display:
                cv2.imshow('Turtle Detection - Enhanced Filtering', annotated_frame)
