...
This project uses `uv` for fast and efficient Python packaging and dependency management, with all configurations centralized in the `pyproject.toml` file.

### Migrating from `requirements.txt`
The project has been migrated from a `requirements.txt`-based workflow to a modern, `pyproject.toml`-centric setup managed by `uv`. This change was implemented to improve organization, speed up dependency resolution, and align with current Python packaging standards.

The `uv init` command was used to create the `pyproject.toml` file, and the dependencies from the old `requirements.txt` were manually migrated into the `[project.dependencies]` section. The now-obsolete `requirements.txt` file has been deleted to maintain a single source of truth for project dependencies.

### Current Workflow
- **Dependencies**: All core project dependencies are listed in `pyproject.toml`.
- **Virtual Environment**: The `uv venv` command is used to create a virtual environment.
- **Installation**: To set up the project, run `uv pip install -e .` to install the project in editable mode along with all its dependencies.

This new setup ensures a cleaner, faster, and more maintainable development environment.
